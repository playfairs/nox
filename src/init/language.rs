...
impl Language {
    ...
    pub fn label(self) -> String {
        ...
        RULES
            .get_or_init(|| InitRules::load().expect("embedded init rules are valid"))
            .language(self)
    ...
    pub fn from_path(path: &Path, rules: &InitRules) -> Option<Self> {
        ...
        rules
            .languages
            .iter()
            .find(|rule| rule.extensions.iter().any(|value| value == extension))
            .map(|rule| rule.language)
    }
    ...
    pub fn is_header(path: &Path, rules: &InitRules) -> bool {
        path.extension()
            ...
            .is_some_and(|extension| {
                rules.languages.iter().any(|rule| {
                    rule.header_extensions
                        .iter()
...
pub fn is_test_path_with_rules(path: &Path, rules: &InitRules) -> bool {
    let names = rules
        .conventions
        .iter()
        .find(|rule| rule.name == "test_directories")
        .map(|rule| &rule.values);
    path.components().any(|component| {
        names.is_some_and(|values| {
            values
                .iter()
                .any(|value| component.as_os_str() == std::ffi::OsStr::new(value))
        })
    })
}
